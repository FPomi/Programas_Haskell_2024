...
posición_personaje = foldPersonaje const (siguiente_posición . const . const) (const id)

-- aux :: Personaje -> Dirección -> Posición -> Dirección
-- componemos la aux que "parsea" los datos para poder usar sig pos con sig pos, 
-- y esa ya devuelve una pos que es lo que pide la firma de la funcion ppal

-- nombre_personaje :: Personaje -> String
-- nombre_personaje = foldPersonaje (const id) const id
nombre_objeto :: Objeto -> String
nombre_objeto = foldObjeto (const . flip id) const id
...
objeto_libre_mas_cercano p u = foldr1 (\x1 x2 -> if distancia p x1 <= distancia p x2 then x1 else x2) (objetos_libres_en u)
